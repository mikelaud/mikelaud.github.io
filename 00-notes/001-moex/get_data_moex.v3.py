...
class Const(object):
    METADATA_FILENAME = 'securities.json'
    DATA_DIR = 'data_moex'
...
def download_url(url, params={}):
    ...
    while True:
        try:
            ...
            print('Retry after ConnectTimeout...')
            continue
        except requests.exceptions.ConnectionError:
            print('Retry after ConnectionError...')
            continue
...
def get_data(symbol, from_date='2000-01-01', till_date='2030-12-31', data_interval=DataInterval.M1, cursor_start=0):
    ...
    return download_url(url, params)
...
def download_metadata_symbols(data_location):
    metadata_location = os.path.join(data_location, Const.METADATA_FILENAME)
    print('Metadta location: {location}'.format(location=metadata_location))
    url = 'https://iss.moex.com/iss/engines/stock/markets/shares/boards/TQBR/securities.json'
    metadata = download_url(url)
    with open(metadata_location, 'w', encoding='utf-8') as f:
...
def main():
    ...
    download_metadata_symbols(data_location)
    print('Done.')
